"""
Comprehensive Benchmark Script for SFT Posttrained Model

Tests the sft_posttrainedpolicy_sft_1 model against:
- Random search configurations
- Fixed baseline configurations
- Different search strategies
- Generates a large comparison table

Usage:
    python benchmark_sft.py --episodes 50
...
def run_benchmark(cfg, num_episodes=50, 
    ...
    print(f"\n{'='*80}")
    print(f"COMPREHENSIVE BENCHMARK: SFT Posttrained Model")
    print(f"{'='*80}")
    ...
    # ========================================
    # 1. SFT Posttrained Model (Learned)
    # ========================================
    if structure_model_path and prompt_model_path:
        print(f"[1] SFT Posttrained Model (Learned)")
        ...
        try:
            ...
            for ep in range(num_episodes):
                ...
                detailed_results.append({
                    "strategy": "SFT Posttrained (Learned)",
    ...
    # ========================================
    # 2. Random Search (Hierarchical)
    # ========================================
    print(f"[2] Random Search (Hierarchical)")
    start_time = time.time()
    ...
    # ========================================
    # 7. Fixed Baseline Configurations
    # ========================================
    print(f"[3] Fixed Baseline Configurations ({len(BASELINE_CONFIGS)} configs)")
    ...
    if results:
        ...
        if detailed_results:
            ...
            output_path = f"benchmark_results_sft_{cfg.DATASET_NAME}_{int(time.time())}.csv"
            detailed_df.to_csv(output_path, index=False)
            print(f"\nDetailed results saved to: {output_path}")
            
            # Save summary table
            summary_path = f"benchmark_summary_sft_{cfg.DATASET_NAME}_{int(time.time())}.csv"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Comprehensive Benchmark for SFT Posttrained Model",
    ...
    parser.add_argument(
        "--structure-model", type=str, 
        default="models/sft_posttrained/structure_policy_sft_1.pt",
        help="Path to structure policy model (.pt file)"
    )
    parser.add_argument(
        "--prompt-model", type=str,
        default="models/sft_posttrained/prompt_policy_sft_1.pt",
        help="Path to prompt policy model (.pt file)"
    )
    parser.add_argument(
...
def main():
    ...
    if not os.path.exists(args.structure_model):
        ...
        print("Skipping learned model benchmark...")
        args.structure_model = None
        args.prompt_model = None
    
    if not os.path.exists(args.prompt_model):
